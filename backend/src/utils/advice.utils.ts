    ...
    if (Array.isArray(category)) {
      for (const cat of category) {
        if (!validCategories.includes(cat as string)) {
          errors.push(`Invalid category: ${cat}. Valid categories are: ${validCategories.join(', ')}`)
        }
      }
    } else if (!validCategories.includes(category as string)) {
      errors.push(`Invalid category: ${category}. Valid categories are: ${validCategories.join(', ')}`)
    }
  }

  // Check for unknown query parameters
  const allowedParams = ['page', 'limit', 'category', 'sortBy', 'order']
 ...
 * @param category The category or categories to filter by
 * @returns A query object for MongoDB
 */
export const buildQueryObject = (category: any): any => {
  const query: any = {}
  if (category) {
...
export const extractQueryParams = (req: Request): { 
  ...
  const { category, page, limit, sortBy = 'createdAt', order = 'desc' } = req.query
  return { category, page, limit, sortBy, order }
}